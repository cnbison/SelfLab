...
| **Phase 2（完整实验，当前）** | 实验报告 + 修订文档 | ✅ 实验代码 + 可能的小工具 | 完整验证 |
| **Phase 3+（系统完善）** | 实验报告 + 应用原型设计 | ✅ 实验代码 + 原型代码 | 应用探索 |

> **当前阶段（2026-06-18）**：**Phase 2（完整实验）** — M2.1 启动中（M1.1~M1.4 已在 2026-06-17 全部通过）。
...
- **DEVELOP.md §四 目录结构**（`sge/` Python 包）是"前瞻性"参考——Phase 3+ 才考虑，不在当前阶段实施
...
| 形态 | 原因 |
|------|------|
| **可复用的 sge/ Python 包** | Phase 3+ 才考虑，超出当前范围（参见 [DEVELOP.md §四 目录结构](../DEVELOP.md) 的"前瞻性"标注） |
...
- 如果 Phase 3+ 需要可复用代码，应**新建仓库**（如 SGE-Prototype），而不是把实验代码重构为包
