...
# 5. 决策结果（基于 §0.5 三原则推导，2026-06-19 填写）

> **本节填表说明**：本次决策由 Bisen 显式委托 Claude 基于 §0.5 三原则锚点推导填写。所有选择均符合三原则评估，如有调整需要回到三原则重新推导。
>
> 完整推导过程见 [discussions/2026-06-19-sge-phase0-closeout-three-principles-anchoring.md](../../discussions/2026-06-19-sge-phase0-closeout-three-principles-anchoring.md) 和 [SGE-Key-Insights.md §洞察 30](../../SGE-Key-Insights.md)。

## 5.1 6 个核心决策点的选择

### 决策点 1：SGE drives 维度清单
**选择**：**候选 B** — `exploration, safety, creativity, connection, autonomy`

**理由**：
- 候选 A（照搬 AiBeing 原生 5 drives）违反原则 2（"SGE 是根，借鉴为 SGE 服务，不能被带歪"）—— AiBeing 的 drives 是为角色扮演场景（Luna）设计的，`play` 和 `expression` 在 SGE 的"涌现"主题下意义不明
- 候选 C（与 values 合并）违反原则 3（过早简化）—— 丢失"本能需求张力"工程机制
- 候选 D（3 层结构）违反原则 3（过早复杂化）—— 调参成本过高
- **候选 B 最契合**：SGE 化语义（`exploration` 比 `novelty` 主动；`creativity` 比 `expression` 更通用）；保留独立 drives 维度；保持 5 drives 的简洁性
- **架构要求**：代码层 drives 维度必须 **schema 化、可配置**——即使阶段 B 用固定清单验证，架构层也要为后续多场景 profile 化留接口（满足原则 1）
...
- 原则 2（SGE 是根）：开放价值空间让"涌现"更自然
- 原则 3（逐步验证）：开放空间更利于发现"价值分化"现象（这正是 SGE 想要研究的）
- [0, 1] 的人为约束会过早限制价值空间——而当前无充分理由要这个约束

### 决策点 3：Phase Transition 阈值
**选择**：**B：扫描 [1.0, 3.0]**（阶段 B 实验中扫描，扫描结果给出实验支撑的默认值）

**理由**：
- SGE 的挫败源是 **values** 而非 AiBeing 的 **drives**，机制不同
- 固定值风险大（AiBeing 的 2.0 不一定适用于 SGE 的挫败源）
- 扫描 [1.0, 3.0] 能给出实验支撑的默认值
- 符合原则 3（逐步验证）

### 决策点 4：Hawking gamma
**选择**：**B：0.01/h**（3 天半衰期，作为阶段 B 起点，实验中可调）

**理由**：
- 1000 epoch 实验窗口就是几天
- AiBeing 默认 0.001/h（半衰期 ~29 天）在几天窗口内几乎无衰减，不适合实验观察
- 先用 0.01/h 跑，看效果再调
- 符合原则 3（逐步验证 + 可调）

### 决策点 5：Crystallize 距离阈值
**选择**：**B：维度归一化**（0.25 / sqrt(N)）

**理由**：
- 体现"产品基座"思维（原则 1）—— 阈值不随维度变化，让架构可扩展
- 这是**反 AiBeing 默认值（固定 0.25）的**——但更可持续
- N=8（AiBeing）时 0.25/sqrt(8) ≈ 0.088；N=12（SGE）时 0.25/sqrt(12) ≈ 0.072——自然地随维度收紧
- 符合原则 2（"SGE 是根"——不照搬 AiBeing 的 magic number）

### 决策点 6：M2.1 阶段 B LLM 选型
**选择**：**A：MiniMax-M3**（阶段 B 沿用）；M2.2/M2.3 阶段再升级（双模型或更大模型）
...
## 5.2 两个元问题的处置（暂不决策 + 架构层留接口）

### 元问题 1：Value 维度"领域适配"
**处置**：**暂不直接决策**，架构层**为 value profile 留接口**

**理由**：
- 满足原则 3（阶段 B 用固定 6 values 验证机制，避免过早分支）
- 满足原则 1（架构层 profile 化为 Phase 3 多场景复用铺路）
- 满足原则 2（不锁死通用 6 values 的核心地位）
- 具体 schema 设计留到 Phase 3（产品化阶段）

### 元问题 2：Meta-values 是否绝对不可修改
**处置**：**暂不直接决策**，架构层**为元配置层留接口**

**理由**：
- 阶段 B 把 meta-values（truth-seeking, freedom）当固定先验（满足"先验证机制"）
- 但**留一个元配置层**允许产品级 override（避免哲学立场锁死工程灵活性）
- 具体 override 边界条件留到 Phase 3 哲学讨论

## 5.3 决策实施时间窗

| 时间窗 | 决策点 | 备注 |
|--------|-------|------|
| **M2.1 阶段 B 启动前** | 1, 2 | drives 清单和 Value scale 是工程基础 |
| **M2.1 阶段 B 实施中** | 3, 4, 5 | 参数类，实验中扫描或按推荐默认值 |
| **M2.2/M2.3 阶段** | 6 | LLM 升级 |
| **Phase 3（产品化）** | 元问题 1, 2 | value profile + meta-values override 具体设计 |
...
| 2026-06-19 | 0.3 | 填写 §5 决策结果（基于三原则推导）：drives=候选 B（SGE 化 5 drives + schema 化）、Value scale=[-1, 1]、Phase Transition=扫描 [1.0, 3.0]、Hawking gamma=0.01/h、Crystallize=维度归一化、LLM=MiniMax-M3；元问题 1、2 暂不决策，架构层留接口 |
...
**最后更新**：2026-06-19
**维护者**：Bisen & Claude
**状态**：✅ §5 决策已填写（基于 §0.5 三原则推导，待 M2.1 阶段 B 实施验证）
