...
def _run_unit_tests() -> bool:
    """兼容层：转调 pytest（Phase 3.2 起的测试已在 sge/tests/unit/test_orchestrator.py）。"""
    import subprocess
    import sys
    result = subprocess.run(
        [sys.executable, '-m', 'pytest',
         'tests/unit/test_orchestrator.py', '-v', '--tb=short'],
...
def _run_orchestrator_persistence_integration_tests() -> bool:
    """兼容层：转调 pytest（已在 sge/tests/unit/test_orchestrator.py）。"""
    import subprocess
    import sys
    result = subprocess.run(
        [sys.executable, '-m', 'pytest',
         'tests/unit/test_orchestrator.py::test_checkpoint_every_auto_triggers',
         'tests/unit/test_orchestrator.py::test_phase_xition_triggers_extra_checkpoint',
         'tests/unit/test_orchestrator.py::test_identity_narrative_triggers_extra_checkpoints',
         'tests/unit/test_orchestrator.py::test_orchestrator_persistence_round_trip_consistent',
         'tests/unit/test_orchestrator.py::test_db_and_student_id_must_be_both_set_or_both_none',
         'tests/unit/test_orchestrator.py::test_student_deleted_does_not_interrupt_step',
         '-v', '--tb=short'],
        capture_output=True, text=True,
    )
    print(result.stdout)
