...
| **Phase 2** 完整实验（M2.1~M2.3） | 🚧 M2.1 阶段 B 启动中 | 实验报告 + 修订文档 | ✅ 实验代码 + 小工具 |
...
| **Phase 2** 完整实验 | 完整童年期（从婴儿到少年，跑完整生命周期） | **它有自我吗？** | 🚧 进行中（M2.1 阶段 B 启动，2026-06-19） |
...
| 子项目 | 目标 | 状态 |
|--------|------|------|
| **SGE（自我生成）** | 验证 AI 能否形成持续自我 | Phase 0 ✅ / Phase 1 ✅ / Phase 2 🚧 M2.1 启动 |
...
**Phase 2（完整实验）当前状态 — M2.1 阶段 B 启动中（2026-06-19）**：
- **阶段 A（基线）已完成（2026-06-18）**：`m21_setup.py` + `m21_baseline.yaml`，4/5 借鉴机制 import OK，5 步最小循环 PASS
- **阶段 B（SGE 化改造）决策已填写**：详见 [research/sge-core/SGE-Phase0-Closeout.md §5 决策结果](./research/sge-core/SGE-Phase0-Closeout.md)
  - drives = 候选 B（`exploration, safety, creativity, connection, autonomy` + 代码层 schema 化）
  - Value scale = [-1, 1]
  - Phase Transition = 扫描 [1.0, 3.0]
  - Hawking gamma = 0.01/h
  - Crystallize = 维度归一化 0.25/sqrt(N)
  - LLM = MiniMax-M3（M2.2/M2.3 再升级）
- **三原则锚点（2026-06-19）**：研究+产品一体两面 / SGE 是根 / 逐步验证扩展——所有未来决策的最高优先级评估维度（洞察 30）

**下一步**：M2.1 阶段 B 实施 — 把 AiBeing 的 5 drives 替换为 SGE 的 5 drives、Value EMA 改造、接入真实 LLM，跑多 seed × 长 epoch 验证。
